...
async function viewApplicantCV(userId) {
  ...
  try {
    ...
    body.innerHTML = `<div style="text-align:center;padding:2rem;color:var(--color-danger-text);">Failed to retrieve profile info: ${escapeHtml(err.message)}<br><small style="color:var(--color-text-muted);">${escapeHtml(err.stack)}</small></div>`;
  }
}
